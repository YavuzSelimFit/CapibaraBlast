export default class Renderer {
    ...
    _loadAssets() {
        ...
        this.gridBg.src = 'assets/grid_bg.png';
    ...
    render(active) {
        ...
        // Grid background - Premium 3D Glass
        if (this.gridBg && this.gridBg.complete) {
            c.save();
            c.globalAlpha = 0.8;
            c.drawImage(this.gridBg, 0, 0, gridW, H);
            c.restore();
        } else {
            // Fallback dots if image not ready
            c.fillStyle = 'rgba(0,0,0,0.1)';
            for (let x = 0; x <= this.grid.width; x++) {
                for (let y = 0; y <= this.grid.height; y++) {
                    c.beginPath(); c.arc(x * this.cellSize, y * this.cellSize, 1, 0, Math.PI*2); c.fill();
                }
            }
        }
